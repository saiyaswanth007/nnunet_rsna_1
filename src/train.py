...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if cfg.get("test"):
        ...
        if checkpoint_callback is not None:
            ckpt_path = getattr(checkpoint_callback, "last_model_path", "") or None
...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    # Save metric_value to file
    save_dir = os.path.join(cfg.log_dir, "metric_values")
    os.makedirs(save_dir, exist_ok=True)
    with open(os.path.join(save_dir, f"metric_value_{cfg.data.fold}.txt"), "w") as f:
